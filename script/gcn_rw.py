...
def prc(pos, neg):
    ...
    return precision_recall_curve(labels, torch.sigmoid(scores))[:-1]
...
def main(mtype, etype):
    ...
    n_features = train_g.nodes['Gene'].data['feature'].shape[1]
    ...
    for epoch in range(1001):
        train_negative_graph = construct_negative_graph(train_g, 1, edge)
        ...
        if not epoch % 1:
            with torch.no_grad():
                test_negative_graph = construct_negative_graph(test_g, 1, edge)
                test_pos_score, test_neg_score = model(test_g, test_negative_graph, test_node_features, edge)
                p, r = prc(test_pos_score.cpu(), test_neg_score.cpu())
                area = auc(r, p)
                if area > max_pr:
    #                torch.save(model, "./model{}".format(time()))
                    max_pr = area
                    max_score_epoch = epoch
                    time_save = time()-start
                    data_save = test_pos_score, test_neg_score
#                print(epoch, loss.item(), *compute_acc(train_pos_score, train_neg_score), area)

    with open("./dwScoreData/lr4_{}.pkl".format(int(time())), "bw") as file:
        pickle.dump(((max_score_epoch, time_save),data_save), file)
    print(max_score_epoch, p, r)
